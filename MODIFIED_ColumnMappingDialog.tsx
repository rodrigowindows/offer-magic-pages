...
import { ArrowRight, Check, X, Settings2, Wand2, Sparkles, Loader2, AlertCircle, Eye, BookOpen } from "lucide-react";
...
import { VisualColumnMatcher } from "@/components/VisualColumnMatcher";
...
const autoDetectField = (csvHeader: string): DatabaseFieldKey | '' => {
  ...
  const mappings: Record<string, DatabaseFieldKey> = {
    ...
    'ownermailingaddress': 'owner_address',
    'ownerfulladdress': 'owner_address',
...
const ColumnMappingDialog = ({
  ...
  // Batch update mappings (used by VisualColumnMatcher)
  const handleBatchMappingChange = (newMappings: ColumnMapping[]) => {
    setMappings(newMappings);
    onMappingChange(newMappings);
  };
  ...
  const handleLoadTemplate = (templateMappings: ColumnMapping[]) => {
    // Match template mappings to current CSV headers
    const newMappings = csvHeaders.map(csvCol => {
      const templateMapping = templateMappings.find(tm => tm.csvColumn === csvCol);
      if (templateMapping) {
        return {
          csvColumn: csvCol,
          dbField: templateMapping.dbField,
          confidence: 'high' as const,
          reason: 'Carregado do template',
        };
      }
      // Keep existing mapping or auto-detect
      const existing = mappings.find(m => m.csvColumn === csvCol);
      return existing || {
        csvColumn: csvCol,
        dbField: autoDetectField(csvCol),
        confidence: 'medium' as const,
        reason: 'Auto-detectado',
      };
    });
    ...
    const matchedCount = newMappings.filter(m =>
      templateMappings.some(tm => tm.csvColumn === m.csvColumn && tm.dbField === m.dbField)
    ).length;

    toast({
      title: "Template Carregado!",
      description: `${matchedCount} colunas mapeadas do template`,
    });
  };
  ...
  const handleAIMapping = async () => {
    ...
    try {
      const result = await mapColumnsWithAI(csvHeaders, csvData, (status) => {
        setAIStatus(status);
      });
      ...
      if (result.success && result.mappings.length > 0) {
        ...
        const fallbackMappings = fallbackToStringMatching(csvHeaders, csvData);
        const newMappings: ColumnMapping[] = csvHeaders.map(csvCol => {
        ...
        <Tabs defaultValue="standard" className="w-full">
          ...
          <TabsContent value="visual">
            {csvData && csvData.length > 0 ? (
              <VisualColumnMatcher
                ...
                onMappingChange={handleBatchMappingChange}
              />
            ) : (
              <div className="text-center py-8 text-muted-foreground">
                Dados do CSV não disponíveis para visualização
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="preview">
            {csvData && csvData.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                Dados do CSV não disponíveis para preview
              </div>
            )}
          </TabsContent>
