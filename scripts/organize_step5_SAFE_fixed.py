...
Move apenas arquivos que NO so do Lovable (Python, CSVs, Docs extras)
MANTM estrutura Lovable intacta na raiz!
...
def create_safe_structure():
    ...
    folders = {
        ...
        "data/archives": "Verses antigas de dados",
        "database": "SQL migrations e setup",
        "docs/setup": "Guias de setup",
        "docs/import": "Guias de importao",
        "docs/technical": "Docs tcnicas",
        "docs/reports": "Relatrios",
    }
...
def move_python_scripts():
    ...
    python_files = [
        ...
        "organize_step5.py",
        "organize_step5_SAFE.py",  # Este prprio script
    ]
...
def move_data_files():
    ...
    for file in Path(".").glob("*.csv"):
        # Pula se for arquivo crtico do Lovable
...
def create_organization_readme():
    """Cria README sobre a organizao"""
    readme_content = """#  Organizao de Arquivos

Este projeto foi reorganizado em {date} para separar:

##  Arquivos Lovable (ficaram na raiz)
```
src/                    # Cdigo React/TypeScript
public/                 # Assets pblicos
...
etc...
```
**ESTES ARQUIVOS NO FORAM MOVIDOS!**
...
- `auto_setup_and_upload.py` - Setup automtico
...
- `import/` - Guias de importao
- `technical/` - Documentao tcnica
- `reports/` - Relatrios e anlises
...
def verify_lovable_structure():
    ...
    if all_ok:
        ...
        print("  ATENO: Arquivos crticos faltando!")
...
def main():
    ...
    try:
        ...
        # 2. Mover apenas arquivos no-Lovable
        move_python_scripts()
        move_data_files()
        move_database_files()
        move_documentation()
        ...
        print("\n" + "=" * 70)
        if lovable_ok:
            print(" REORGANIZAO COMPLETA - LOVABLE INTACTO!")
        else:
            print("  REORGANIZAO COM AVISOS - VERIFICAR MANUALMENTE!")
        print("=" * 70)
        ...
        print(f"\n ERRO: {e}")
        print("   Nenhum arquivo crtico foi movido.")
