...
const buildPropertyPayload = (property: Record<string, any>) => ({
  ...
  const scoreProperty = useCallback(async (property: Record<string, any>): Promise<AIScoreResult | null> => {
    return withAILoading(setLoading, 'AI Score', toast, async () => {
      const { data, error } = await supabase.functions.invoke('ai-score-property', {
        body: { property: buildPropertyPayload(property) },
      });
...
/**
 * Static version for auto-scoring (no hooks, fire-and-forget).
 * Used when a property loads without an AI score.
 */
export const useAIScoreStatic = async (property: Record<string, any>): Promise<AIScoreResult | null> => {
  try {
    const { data, error } = await supabase.functions.invoke('ai-score-property', {
      body: { property: buildPropertyPayload(property) },
    });
