...
export const BatchOfferPrintDialog = ({ properties, open, onOpenChange }: BatchOfferPrintDialogProps) => {
  ...
  // ── Address audit: which source the shared resolver picked ──────
  const audit = properties.reduce(
    (acc, p) => {
      const parts = resolveMailingAddress(p);
      const hasName = !!(p.owner_name && p.owner_name.trim());
      if (parts.source === 'confirmed') acc.confirmed.push(p);
      else if (parts.source === 'owner_address') acc.ownerAddr.push(p);
        ...
        <div className="border rounded-lg p-4 max-h-[60vh] overflow-y-auto bg-muted/30">
          <div ref={printRef}>
            {properties.map((property, index) => {
              const parts = resolveMailingAddress(property);
              return (
                <div key={property.id} className="letter-page mb-8 last:mb-0">
                  ...
                  <CashOfferLetter
                    ...
                    ownerName={parts.name}
                    language={language}
                    source="letter"
                    mailingAddress={parts.line1}
                    mailingCity={parts.city}
                    mailingState={parts.state}
                    mailingZip={parts.zip}
                  />
                </div>
              );
            })}
