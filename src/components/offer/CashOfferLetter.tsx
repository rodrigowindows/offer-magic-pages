...
export const CashOfferLetter = ({
  ...
  const headline = (language === "es" ? tpl.headlineEs : tpl.headlineEn) ?? t.headline;
  const sub = (language === "es" ? tpl.subEs : tpl.subEn) ?? t.subheadline;
  ...
  return (
    <div className="cash-offer-letter-wrapper print:w-full print:mx-auto">
      ...
      >
        <div
          className={`letter-header p-6 print:py-1.5 print:px-3 text-center ${tpl.headerClass}`}
        >
        ...
        <div className="p-6 space-y-6 print:px-6 print:py-5 print:space-y-5">
          <div className="text-center space-y-2 print:space-y-1.5">
            ...
            {tpl.accent && (
              <p className="text-xs uppercase tracking-widest text-muted-foreground print:text-[10px]">
                {tpl.accent}
              </p>
            )}
            <h1 className={`leading-tight ${tpl.headlineClass}`}>{headline}</h1>
            <p className="text-base text-muted-foreground print:text-sm">{sub}</p>
            {/* PROPERTY ADDRESS — TOP */}
            <p className="text-sm font-medium text-primary pt-1 print:text-[12px]">
              {fullAddress}
            </p>
          </div>

          <div className={`letter-offer-box relative rounded-xl p-6 print:p-5 text-center ${tpl.offerBoxClass}`}>
            {tpl.showUrgencyPill && (
            ...
            <p className="text-base text-muted-foreground mb-2 mt-3 print:text-sm">
              {t.cashOffer}
            </p>
            <p className="text-3xl print:text-2xl font-black text-primary leading-tight">
              {language === "es"
                ? "Llame o Escanee para Ver Su Oferta"
                : "Call or Scan to See Your Offer"}
            </p>
          </div>
          ...
          {/* PHONE + QR — always visible across all templates */}
          <div className={`letter-cta rounded-xl p-5 print:p-4 text-center space-y-3 ${tpl.ctaClass}`}>
            ...
            <div className="pt-3 border-t border-primary-foreground/20">
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm font-bold leading-tight print:text-xs">{t.qrCallout}</p>
              </div>
            </div>
          </div>
