...
export const InlineCompsList = ({ comps, onOpenComps, subjectSqft }: InlineCompsListProps) => {
  ...
  return (
    <div className="border-t pt-3 space-y-2" data-section="comps-list">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
          <BarChart3 className="h-3.5 w-3.5" />
          Comps Salvos ({comps.length})
        </p>
        <Button variant="ghost" size="sm" onClick={onOpenComps} className="text-xs text-blue-600 hover:text-blue-800 h-6 px-2" data-action="add-comp">
          + Adicionar
        </Button>
      </div>
                ...
                <TableRow key={comp.id} data-comp-id={comp.id}>
                  <TableCell className="py-1.5 text-xs">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <Badge variant="outline" className="text-[8px] shrink-0 uppercase">{comp.source}</Badge>
